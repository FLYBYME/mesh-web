...
- [x] **A7.0 ★★ `Input` cannot be controlled, and no text input can be cleared.** *(fixed 2026-09-05)* Added `apply` on `Input` assigning live DOM `.value` (with caret-preserving inequality check) and `.checked` properties.
      ...
      So `props: { value: () => draft() }` fails *silently* — the text stays in the box after submit.
...
- [x] **A7.0b `activate` fires on Space, so it cannot be bound to an `Input`.** *(fixed 2026-09-05)* Added `spaceIsTextInput` to `ComponentDefinition` and suppressed `activate` on Space for text input elements.
      ...
      non-pointer path*, not *every key is an action*. **S**
...
- [ ] **A8.18 ★★★ The renderer is a driver and cannot be swapped; a primitive is not a driver and
      ...
      **The shape to copy is NT's I/O manager, and half of it is already here.** The kernel has an
      ...
      2. **[x] One driver registration point** covering the renderer, storage and what comes next, so
         `StorageProvider` stops being a special case.
      3. **[x] Then the missing drivers**, driven by what asks: a window driver (`window.open` and
